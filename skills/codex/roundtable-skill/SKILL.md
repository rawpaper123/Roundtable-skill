...
- The current coding terminal is the Executor.
- The Executor can be Codex, Claude Code, Kimi Work, Cursor, Windsurf, or another agentic terminal.
- The Executor is final arbiter, implementer, Git owner, verifier, PR owner, deploy owner, and rollback owner.
...
1. Inspect the repo/task enough to know the real scope.
2. Pick 1-5 available Lingtai agents. At least one is required; add more when the task benefits from more expert angles.
3. Assign each a task-specific expert angle.
...
8. Executor executes the smallest safe change.
9. Verify with the smallest meaningful checks.
10. Report:
...
- Git/PR/deploy/rollback state is clear.
